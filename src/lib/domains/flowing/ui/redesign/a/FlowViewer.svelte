<script lang="ts">
  ...
  const { view, book, fill = 'screen', panel, anchors = NO_ANCHORS, onLift }: Props = $props();
  ...
  const awake = $derived(chromeShown(barsAsked, focus.held || panelOpen));
...
<div
  class={[
    'flow-viewer row gap-0 overflow-hidden surface-bg',
...
>
  <div class="reading overlay-host relative flex-1 min-h-0">
    ...
    <header
      class={[
        'pin-top z-raised row wrap items-center gap-3 px-4 py-2 surface border-b hushable',
        { 'is-hushed': !awake },
      ]}
      inert={!awake}
      bind:this={topBar}
    >
      ...
      <div class="row items-end gap-2 flex-1">
        <h1 class="text-sm weight-normal truncate" lang={book.language}>{book.title}</h1>
    ...
    <footer
      class={[
        'pin-bottom z-raised row items-center gap-3 px-4 py-2 surface border-t hushable',
        { 'is-hushed': !awake },
      ]}
      inert={!awake}
      bind:this={bottomBar}
    >
      <div class="row gap-1 shrink-0" role="group" aria-label="Turn the page">
        {#each order as turn, slot (turn)}
          {@const Icon = ICONS[slot] ?? ChevronRight}
          <Button size="sm" square pill disabled={!reading} onclick={() => view.turn(turn)}>
            <Icon class="btn-icon" />
            <span class="visually-hidden">{TURN_LABELS[turn]}</span>
          </Button>
        {/each}
      </div>

      <p
        class={['text-xs shrink-0', progress.kind === 'unknown' ? 'text-faint' : 'mono text-muted']}
      >
        {marker}
      </p>
    ...
    <FlowSettingsDialog bind:open={settingsOpen} {settings} onchoose={chooseSettings} />
    ...
    {#if curtain.kind === 'opening'}
      <div class="curtain overlay-fill z-sticky col items-center gap-3 p-5 surface-bg">
