...
describe('selectImageEntries', () => {
	...
	it('rejects .DS_Store and Thumbs.db anywhere in the tree', () => {
		expect(selectImageEntries(['.DS_Store', 'vol1/.ds_store', 'Thumbs.db', 'vol1/thumbs.db'])).toEqual(
			[]
		);
	});

	it('rejects a directory entry that ends in a slash', () => {
		expect(selectImageEntries(['vol1/', 'weird.jpg/', 'vol1/page1.jpg'])).toEqual(['vol1/page1.jpg']);
	});
