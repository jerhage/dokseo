<script lang="ts">
  ...
  import ArrivalBar from '$lib/domains/recognition/ui/capture/redesign/ArrivalBar.svelte';
  import CapturePalette from '$lib/domains/recognition/ui/capture/CapturePalette.svelte';
  import CapturePanel from '$lib/domains/recognition/ui/capture/redesign/CapturePanel.svelte';
  ...
  beforeNavigate((navigation) => {
    ...
    if (to === undefined || !to.pathname.startsWith('/read/')) return;

    navigation.cancel();
    void goto(`/preview/b${to.pathname}${to.search}`, {
      replaceState: true,
      keepFocus: true,
      noScroll: true,
    });
